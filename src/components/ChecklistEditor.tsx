...
// Cada item é uma entidade: título + descrição opcional, com reordenação.
// Serializa tudo num hidden input "checklistItems" — a action faz replace-all.

export type ChecklistDraft = { titulo: string; descricao: string };
...
export function ChecklistEditor({ initialItems }: { initialItems: ChecklistDraft[] }) {
  ...
  const update = (i: number, field: keyof ChecklistDraft, value: string) =>
    setItems((prev) => prev.map((item, idx) => (idx === i ? { ...item, [field]: value } : item)));

  const add = () => setItems((prev) => [...prev, { titulo: "", descricao: "" }]);
  ...
  // só itens com título contam — os vazios são descartados no save
  const serialized = JSON.stringify(
    items.filter((i) => i.titulo.trim()).map((i) => ({ titulo: i.titulo.trim(), descricao: i.descricao.trim() })),
  );
    ...
    <div className="flex flex-col gap-2">
      ...
      {items.length === 0 && (
        <p className="rounded-lg border border-dashed border-zinc-300 p-3 text-xs text-zinc-400 dark:border-zinc-700">
          Nenhum item ainda — cada item vira um objetivo marcável na tela de discagem.
        </p>
      )}
        ...
        >
          ...
          <div className="flex flex-1 flex-col gap-1.5">
            <input
              ...
              placeholder="objetivo (ex.: Falar com o decisor)"
              className={fieldClasses}
            />
            <input
              value={item.descricao}
              onChange={(e) => update(i, "descricao", e.target.value)}
              placeholder="detalhe opcional (ex.: pegar nome + melhor horário)"
